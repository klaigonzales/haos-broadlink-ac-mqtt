...
def discover(timeout=None, bind_to_ip=None):
	if bind_to_ip is None:
		try:
			...
			try:
				bind_to_ip = socket.gethostbyname(socket.gethostname())
			except Exception:
				bind_to_ip = '0.0.0.0'
...
class ac_db(device):
	...
	@staticmethod
	def decode_ambient_temp(response_payload):
		"""Ambient temperature in whole degrees plus one decimal.

		Byte 15 low 5 bits are the whole degrees and bit 6 adds 32, which is what
		lets the unit report above 31C at all; byte 31 low 5 bits are the tenths.
		Only the 5 bits were read before, so a hot room wrapped around and the
		decimal @Anonym-tsk logged for years was never used. (+32 via dvb6666)
		"""
		return (
			((response_payload[15] >> 6) & 0b00000001) * 32
			+ (response_payload[15] & 0b00011111)
			+ (response_payload[31] & 0b00011111) / 10.0
		)
	...
	def get_ac_info(self):
		...
		if err == 0:
			...
			ambient_temp = self.decode_ambient_temp(response_payload)

			self.logger.debug("Ambient Temp Decimal: %s" % float(response_payload[31] & 0b00011111) ) ## @Anonym-tsk
			self.logger.debug("Ambient Temp: %s" % ambient_temp)

			##Kept truthy rather than "is not None": a unit that does not report
			##ambient at all sends zeroed bytes, and holding the last good reading
			##beats publishing a fake 0C. Costs us a genuine reading of exactly
			##0.0C, which is the cheaper of the two errors.
			if ambient_temp:
	...
	def make_nice_status(self,status):
		...
		##get_key hands back the raw int when the value is unmapped, and the AC does
		##report 0 (the commented-out STOP). HA then logs "Invalid swing_modes mode:
		##0" and drops the entity, so fall back to a mode it does know. (@ybeapps)
		fixation_v = self.get_key(self.STATIC.FIXATION.VERTICAL.__dict__,status['fixation_v'])
		status_nice['fixation_v'] = fixation_v if isinstance(fixation_v,str) else 'AUTO'
		...
		##Make fanspeed logic
		##Same story as fixation_v above: get_key hands back the raw int for a
		##speed the table does not name, and .title() on an int ends the poll with
		##an AttributeError. NONE is named but was never announced to HA either.
		##Both become a speed the entity actually lists. (@isw866)
		fanspeed = self.get_key(self.STATIC.FAN.__dict__,status['fanspeed'])
		if not isinstance(fanspeed,str) or fanspeed == 'NONE':
			self.logger.debug("Fan speed %s is not one HA was told about, reporting AUTO" % str(fanspeed))
			fanspeed = 'AUTO'
		status_nice['fanspeed']  = fanspeed
		status_nice['fanspeed_homeassistant']  = fanspeed.title()
